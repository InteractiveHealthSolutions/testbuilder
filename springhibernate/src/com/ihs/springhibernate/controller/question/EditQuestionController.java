...
public class EditQuestionController
{
	...
	public ModelAndView getEditQuestion(@RequestParam("id") String questionId)
	{
		...
		if (userSession != null)
		{
			...
			{
				Question editQuestion = QuestionDAO.getQuestion(QuestionDAO.By.ID, questionId, QuestionDAO.FetchType.EAGER);

				modelAndView = new ModelAndView(resources.getFOLDER_QUESTION() + "/" + resources.getJSP_EDIT_QUESTION());
	...
	// {
	// QuestionData questionData = QuestionDataDAO.getQuestionData(QuestionDataDAO.By.ID, String.valueOf(imageId), QuestionDataDAO.FetchType.EAGER);
	// // Image image = new //obtain Image instance by id somehow from DAO/Hibernate
	//
	// return questionData.getFile().getBytes();
	// }

	@RequestMapping(value = "/editquestion", method = RequestMethod.POST)
	public ModelAndView submitQuestion(@ModelAttribute("editQuestion") @Valid Question editQuestion, BindingResult result)
	{
		...
		if (userSession != null)
		{
			...
			{
				modelAndView = new ModelAndView("redirect:/" + resources.getFOLDER_QUESTION() + "/" + resources.getJSP_VIEW_QUESTION() + "?id=" + editQuestion.getId());
