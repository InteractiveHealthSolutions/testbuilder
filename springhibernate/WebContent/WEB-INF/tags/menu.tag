	...
	<ul class="nav nav-pills nav-stacked">
		...
		<c:forEach var="right" items="${user.getRole().getPrivilegeList()}">
			...
			<c:if test="${ right.getId() == 1}">
				...
				<c:url var="home"
					value="/${Resource.JSP_HOME}">
				</c:url>

				<li><a href="<c:out value="${home}"/>">Home</a></li>

				<c:url var="manageUser"
					value="/${Resource.FOLDER_USER}/${Resource.JSP_MANAGE_USER}">
				</c:url>

				<li><a href="<c:out value="${manageUser}"/>">Account
						Management</a></li>
			...
			<c:if test="${right.getId() == 2 }">
				...
				<c:url var="manageQuestion"
					value="/${Resource.FOLDER_QUESTION}/${Resource.JSP_MANAGE_QUESTION}">
				</c:url>


				<li><a href="<c:out value="${manageQuestion}"/>">Manage
						Questions</a></li>


				<c:url var="createQuestion"
					value="/${Resource.FOLDER_QUESTION}/${Resource.JSP_CREATE_QUESTION}">
				</c:url>

				<li><a href="<c:out value="${createQuestion}"/>">Create New
						Question</a></li>

				<c:url var="createCategory"
					value="/${Resource.FOLDER_QUESTION}/${Resource.JSP_ADD_CATEGORY}">
				</c:url>
				<li><a href="<c:out value="${createCategory}"/>">Add New
						Category</a></li>
				...
				<li><a href="<c:out value="${createTestScheme}"/>">Create
						Test Scheme</a></li>

				<c:url var="selectScheme"
					value="/${Resource.FOLDER_TEST}/${Resource.JSP_SELECT_SCHEME}">
				</c:url>

				<li><a href="<c:out value="${selectScheme}"/>">Generate
						Test</a></li>
			...
			<c:if test="${ right.getId() == 4}">
				...
				<c:url var="takeTest"
					value="/${Resource.FOLDER_TEST}/${Resource.JSP_TAKE_TEST_LIST}">
				</c:url>
		...
		<c:url var="logoutUser"
			value="/${Resource.FOLDER_USER}/${Resource.JSP_LOGOUT_USER}">
		</c:url>
